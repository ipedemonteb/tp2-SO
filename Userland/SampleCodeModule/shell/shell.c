...
void check_command() {
  ...
  if (command_count == 2) {
    ...
    if (c == NULL) {
      ...
      uint16_t pid = create_process(c->function, command_argc,
                                    command_tokens + 1, c->name, !bg);
  ...
  if (c == NULL) {
    ...
    uint16_t pid = create_process(c->function, command_argc, command_tokens + 1,
                                  c->name, !bg);
    running_processes_pids[pid] = 1 + bg;
    nice(pid, bg ? 3 : 4);
  }
...
void handle_up_down(int8_t (*condition)(string_arrayADT),
                    char *(*fn)(string_arrayADT, uint16_t *), char *key) {
...
void launchShell() {
  ...
  while (!exitFlag) {
    ...
    case '\t':
      int8_t index;
      if (current_command_length == 1 &&
          (index = getCommandIdx(current_command[0])) >= 0 &&
          commands[index]->name) {
    ...
    default:
      write(TERMINAL, &key, 1);
      insert_letter(current_command, current_command_length, key,
                    current_command_pos);
      current_command_length++;
      current_command_pos++;
      break;
