...
static uint8_t has_next_pipe(pipe_t *p) { return p->last > p->current; }
...
int64_t read(uint8_t bd, char *arr, int64_t size) {
  ...
  while (count < size) {
    if (!has_next_pipe(pipe)) {
      pipe->blocked_read[pcb->pid / 64] =
          set_n_bit_64(pipe->blocked_read[pcb->pid / 64], pcb->pid % 64);
      block(pcb->pid);
    } else {
      if (pipe->buff[pipe->current % BUFF_MAX] ==
          EOF) {
        pipe->current++;
        break;
      }
...
int64_t write(uint8_t bd, char *buffer, int64_t size) {
  ...
  while (count < size) {
    if (pipe->last - pipe->current >= BUFF_MAX) {
      pipe->blocked_write[pcb->pid / 64] =
          set_n_bit_64(pipe->blocked_write[pcb->pid / 64], pcb->pid % 64);
      block(pcb->pid);
    }
...
uint64_t get_available() { return avail; }

pipe_t *get_pipe_by_id(uint8_t id) { return &pipes[id]; }

pipe_t *get_keyboard_buffer() { return &pipes[KEYBOARD]; }

pipe_t *get_terminal_buffer() { return &pipes[TERMINAL]; }

void keyboard_ready() { pipe_ready(&pipes[0], 1); }
