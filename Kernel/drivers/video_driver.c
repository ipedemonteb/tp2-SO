...
void printInPopUp(int8_t *msg, uint16_t x, uint16_t y) {
  ...
  font_size = 1;
  drawString(msg, x + TOP_LEFT_X / 8 + LEFT_MARGIN,
             y + TOP_LEFT_Y / 16 + TOP_MARGIN, WHITE, BLACK);
  font_size = aux;
}

void clearPopUp() {
  uint8_t *framebuffer = (uint8_t *)(uintptr_t)VBE_mode_info->framebuffer;
  uint64_t offset = (TOP_LEFT_X * ((VBE_mode_info->bpp) / 8)) +
                    (TOP_LEFT_Y * VBE_mode_info->pitch);
  ...
  for (uint32_t i = 0; i < POP_UP_HEIGHT; i++) {
    for (uint32_t j = 0; j < POP_UP_WIDTH * BYTES_PER_PIXEL; j += 3) {
      if (i < 2 || i > POP_UP_HEIGHT - 3 || j < 2 * BYTES_PER_PIXEL ||
          j > (POP_UP_WIDTH - 3) * BYTES_PER_PIXEL) {
...
void resetPopUp() {
  ...
  for (uint32_t i = 0; i < POP_UP_HEIGHT; i++) {
    for (uint32_t j = 0; j < POP_UP_WIDTH * BYTES_PER_PIXEL; j += 3) {
      framebuffer[offset + i * width + j] =
          popUpWindowSave[i * POP_UP_WIDTH * BYTES_PER_PIXEL + j];
      framebuffer[offset + i * width + j + 1] =
          popUpWindowSave[i * POP_UP_WIDTH * BYTES_PER_PIXEL + j + 1];
      framebuffer[offset + i * width + j + 2] =
          popUpWindowSave[i * POP_UP_WIDTH * BYTES_PER_PIXEL + j + 2];
    }
  }
}
...
void drawchar(int8_t c, uint32_t x, uint32_t y, uint32_t fgcolor,
              uint32_t bgcolor) {
  x *= 8 * font_size;
  y *= 16 * font_size;
  uint32_t cx, cy;
    ...
    for (cx = 0; cx < 8 * font_size; cx += font_size) {
      for (uint8_t i = 0; i < font_size; i++) {
        for (uint8_t j = 0; j < font_size; j++) {
          putPixel(__font_bitmap__[((int)c - 32) * 16 + cy / font_size] &
                           mask[cx / font_size]
                       ? fgcolor
                       : bgcolor,
                   x + cx + j, y + cy + i);
...
void drawString(int8_t *string, uint32_t x, uint32_t y, uint32_t fgcolor,
                uint32_t bgcolor) {
  uint32_t w = x, h = y;
  while (*string != 0) {
...
uint64_t getWidth() { return VBE_mode_info->width; }

uint64_t getHeight() { return VBE_mode_info->height; }
