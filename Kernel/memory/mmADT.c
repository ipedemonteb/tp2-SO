...
void *mm_alloc(mmADT memory_man, uint64_t size) {
  ...
  if (remaining > sizeof(struct block_data)) {
    block_data_ptr new_free_block =
        (block_data_ptr)((uint8_t *)iter->block + size);
    new_free_block->block =
        (void *)((uint8_t *)new_free_block + sizeof(struct block_data));
...
void mm_free(mmADT memory_man, void *mem) {
  ...
  block_data_ptr block_freed =
      (block_data_ptr)((uint8_t *)mem - sizeof(struct block_data));
...
uint64_t mm_avail(mmADT memory_man) { return memory_man->avail; }

uint64_t mm_size(mmADT memory_man) { return memory_man->size; }